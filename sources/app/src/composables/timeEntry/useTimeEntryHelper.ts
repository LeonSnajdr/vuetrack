...
export function useTimeEntryHelper() {
    ...
    const createDefaultTimeEntry = (defaultValues?: Partial<Nullable<TimeEntryCreateContract>>): Nullable<TimeEntryCreateContract> => {
        const newestTimeEntry = maxBy(timeEntries.value, (x) => x.dateEnded);
        const resolvedStartTime = defaultValues?.dateStarted ?? newestTimeEntry?.dateEnded ?? new Date(new Date().setHours(8, 0, 0, 0));
        const presetDurationMinutes = activePreset.value?.durationMinutes;
        const resolvedEndTime =
            defaultValues?.dateEnded ??
        ...
        return {
            ...
            dateStarted: resolvedStartTime,
            dateEnded: resolvedEndTime,
