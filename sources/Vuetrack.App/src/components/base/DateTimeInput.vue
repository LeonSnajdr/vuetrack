...
const onTimeInputBlur = (event: FocusEvent): void => {
    commitTimeInput();
...
const closeTimeMenu = (): void => {
    isTimeMenuOpen.value = false;
};

const onTimePickerPointerDown = (): void => {
...
const onDateInputKeydown = async (event: KeyboardEvent): Promise<void> => {
...
const isTabIndexMinusOne = (): boolean => {
    const tabIndexAttr = attrs.tabindex ?? attrs.tabIndex;

    return `${tabIndexAttr ?? ""}`.trim() === "-1";
};

const focusTimeInputAndSelectAll = (): void => {
    timeInputRef.value?.focus?.();
    getTimeInputElement()?.select();
};

const getTimeInputElement = (): HTMLInputElement | null => {
...
const shouldKeepTimeMenuOpen = (event: FocusEvent): boolean => {
...
const isElementInsideTimeInputOrMenu = (element: HTMLElement): boolean => {
    const timeInputElement = timeInputRef.value?.$el;
...
const parseTimeInput = (value: string): { hours: number; minutes: number } | null => {
...
const toTimeParts = (hoursText: string, minutesText: string): { hours: number; minutes: number } | null => {
...
const updateDateTime = (hours: number, minutes: number): void => {
...
const getTimeUpdateBaseDate = (): Date | null => {
    if (isFullMode.value) {
...
const mergeDateWithExistingTime = (nextDate: Date, currentDateTime: Date): Date => {
...
const isElementInsideDateInput = (element: HTMLElement): boolean => {
...
const isSameOrBeforeDate = (value: Date | null, comparison: Date): boolean => {
    if (value === null) {
        return false;
    }
...
const getDateOnlyTime = (value: Date): number => {
